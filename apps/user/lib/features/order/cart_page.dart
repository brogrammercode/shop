...
import 'package:user/features/order/order.constant.dart';
...
class _CartPageState extends State<CartPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<OrderCubit, OrderState>(
      ...
      body: Column(
        children: [
          SizedBox(height: statusBarHeight),          _buildHeader(state),
          Expanded(
  ...
  Widget _buildHeader(OrderState state) {
    return Container(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Row(
                    children: [
                      Text(
                        '30-35 mins to ${(state.activeAddress?.title ?? '')}',
                        style: TextStyle(
                          fontSize: 11.sp,
                      ...
                      Expanded(
                        child: Text(
                          ' | ${(state.activeAddress?.fullAddress ?? '')}',
                          maxLines: 1,
  ...
  Widget _buildJioSaavnPromo(OrderState state) {
    final double itemTotal = state.calculateItemTotal;
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (!isUnlocked && !state.jioSaavnAdded)
                      Padding(
              ...
              Column(
                children: [
                  GestureDetector(
                    onTap: isUnlocked
                        ? () {
                            context.read<OrderCubit>().toggleJioSaavn();
                          }
                        : null,
                    child: Container(
                      padding: EdgeInsets.symmetric(horizontal: 14.w, vertical: 6.h),
                      decoration: BoxDecoration(
                        color: state.jioSaavnAdded ? const Color(0xFFE8F5E9) : AppColors.pureWhite,
                        borderRadius: BorderRadius.circular(20.r),
                        border: Border.all(
                          color: state.jioSaavnAdded
                              ? AppColors.primaryGreen
                              : (isUnlocked ? AppColors.textPrimary : AppColors.borderGrey),
                        ),
                      ),
                      child: state.jioSaavnAdded
                          ? Row(
  ...
  Widget _buildCartItemsList(OrderState state) {
    return Container(
      color: AppColors.pureWhite,
      padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 12.h),
      child: Column(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () async {
                              ...
                              if (!mounted) return;
                              if (result != null) {
                                context.read<OrderCubit>().updateItemQuantity(item.id, result.quantity, result.subItems);
                              }
                            },
                            child: Row(
                              children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              GestureDetector(
                                onTap: () {
                                  context.read<OrderCubit>().updateItemQuantity(item.id, item.quantity - 1, item.subItems);
                                },
  ...
  Widget _buildDiscountChip(OrderState state) {
    final double itemTotal = state.calculateItemTotal;
    if (itemTotal >= 299) {
                          ...
                          child: GestureDetector(
                            onTap: () {
                              final item = CartItemModel(
                                ...
                                context.read<OrderCubit>().addItem(item);
                            },
                            child: Container(
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          AppButton(
            onPressed: () {
              context.read<OrderCubit>().toggleGold();
            },
            backgroundColor: state.goldApplied ? const Color(0xFFE8F5E9) : AppColors.pureWhite,
            text: state.goldApplied ? 'APPLIED' : 'APPLY',
  ...
  Widget _buildStickyFooter(OrderState state) {
    return Container(
      padding: EdgeInsets.all(16.w),
      decoration: BoxDecoration(
        color: AppColors.pureWhite,
          ...
          GestureDetector(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  (state.selectedPaymentMethod?.name ?? 'Select Payment'),
                  style: TextStyle(
                    fontSize: 13.sp,
                    fontWeight: FontWeight.w900,
                    color: AppColors.textPrimary,
  ...
  void _selectPaymentMethod() async {
    ...
    if (!mounted) return;
    if (result != null) {
      context.read<OrderCubit>().setSelectedPaymentMethod(result);
    }
  }
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildAddressSection(true, setModalState, context),
                              SizedBox(height: 24.h),
                              _buildAddressSection(false, setModalState, context),
                            ],
                          ),
                        ),
                      ),
  ...
  Widget _buildAddressSection(bool delivers, StateSetter setModalState, BuildContext context) {
        ...
        Column(
          children: list.map((addr) {
            return GestureDetector(
              onTap: delivers
                  ? () {
                      context.read<OrderCubit>().setActiveAddress(addr);
