...
class _SearchResultPageState extends State<SearchResultPage> {
  ...
  double get _totalCartPrice {
    double total = 0;
    for (final rec in <SearchResultRestaurant>[]) {
      final qty = _cartQuantities[rec.id] ?? 0;
      total += rec.price * qty;
    }
    for (final large in <SearchResultLargeRestaurant>[]) {
      ...
      child: Column(
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemCount: (<SearchResultRestaurant>[]).length,
            ...
            itemBuilder: (context, index) {
              final item = (<SearchResultRestaurant>[])[index];
      ...
      child: Column(
        ...
        children: [
          ...
          ListView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            itemCount: (<SearchResultLargeRestaurant>[]).length,
            itemBuilder: (context, index) {
              final item = (<SearchResultLargeRestaurant>[])[index];
              final qty = _cartQuantities[item.id] ?? 0;
              return GestureDetector(
  ...
  Widget _buildCartBar(BuildContext context) {
    ...
    for (final item in <SearchResultRestaurant>[]) {
    ...
    if (firstName.isEmpty) {
      for (final item in <SearchResultLargeRestaurant>[]) {
        if (item.id == firstAddedId) {
          firstName = item.name;
          firstImageUrl = item.imageUrl;
          break;
