...
class _KdsTerminalPageState extends State<KdsTerminalPage> {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        backgroundColor: const Color(
          0xFF121212,
        ), // High contrast dark mode for kitchen
        appBar: AppBar(
          backgroundColor: const Color(0xFF1E1E1E),
          elevation: 0,
          iconTheme: const IconThemeData(color: Colors.white),
          title: Text(
            PosConstant.KDS_TITLE,
            style: TextStyle(
              fontSize: 18.sp,
        ...
        body: BlocBuilder<PosKdsCubit, PosKdsState>(
          builder: (context, state) {
            ...
            final newKots = state.kots
                .where((k) => k.status == 'PENDING')
                .toList();
            final prepKots = state.kots
                .where((k) => k.status == 'PREPARING')
                .toList();
            final readyKots = state.kots
                .where((k) => k.status == 'COMPLETED')
                .toList();
              ...
              children: [
                ...
                newKots.isEmpty
                    ...
                    : ListView(
                        ...
                        children: newKots
                            .map(
                              (k) => _buildModernKotCard(
                                k,
                                Colors.redAccent,
                                'Start Preparing',
                                () => context
                                    .read<PosKdsCubit>()
                                    .updateKOTStatus(k.id, 'PREPARING'),
                              ),
                            )
                            .toList(),
                      ),
                // PREPARING TAB
                prepKots.isEmpty
                    ? _buildEmpty('No orders preparing.')
                    : ListView(
                        padding: EdgeInsets.all(16.w),
                        children: prepKots
                            .map(
                              (k) => _buildModernKotCard(
                                ...
                                () => context
                                    .read<PosKdsCubit>()
                                    .updateKOTStatus(k.id, 'COMPLETED'),
                              ),
                            )
                            .toList(),
                      ),
                // READY TAB
                readyKots.isEmpty
                    ? _buildEmpty('No orders waiting for pickup.')
                    : ListView(
                        padding: EdgeInsets.all(16.w),
                        children: readyKots
                            .map(
  ...
  Widget _buildModernKotCard(
    KitchenOrderTicketModel kot,
    Color accentColor,
    String actionText,
