...
class _AuthPageState extends State<AuthPage> {
          ...
          BlocListener<CoreHrCubit, CoreHrState>(
            ...
            listener: (context, state) {
              if (state.sendOtpInfo.status == OperationStatus.success) {
                Fluttertoast.showToast(msg: HrConstant.OTP_SENT_SUCCESSFULLY);
                if (!_otpSent) {
                  setState(() {
                    _otpSent = true;
                  });
    ...
    return Column(
      ...
      children: [
        ...
        BlocBuilder<CoreHrCubit, CoreHrState>(
          buildWhen: (prev, curr) =>
              prev.sendOtpInfo.status != curr.sendOtpInfo.status,
          builder: (context, state) {
            final isSending =
                state.sendOtpInfo.status == OperationStatus.loading;
            if (isSending) {
              return SizedBox(
                width: 16.w,
                height: 16.w,
            ...
            return GestureDetector(
              onTap: _canResend
                  ? () {
                      ...
                      context.read<CoreHrCubit>().sendOtp(
              ...
              child: Text(
                _canResend
                    ? HrConstant.RESEND_OTP
                    : HrConstant.RESEND_OTP_IN(_resendSeconds),
                style: TextStyle(
                  fontSize: 12.sp,
                  fontWeight: FontWeight.w600,
                  color: _canResend
    ...
    return BlocBuilder<CoreHrCubit, CoreHrState>(
      builder: (context, state) {
        ...
        return AppButton(
          ...
          onPressed: isLoading
              ...
              : () {
                  ...
                  if (!_otpSent) {
                    ...
                    if (otp.isEmpty) {
                      Fluttertoast.showToast(msg: HrConstant.PLEASE_ENTER_OTP);
                      return;
                    }
                    context.read<CoreHrCubit>().login(
                      '${_selectedCountry.dialCode}$phone',
                      otp,
          ...
          text: _otpSent ? HrConstant.VERIFY_OTP : HrConstant.CONTINUE_TEXT,
  ...
  Widget _buildSocialButtons() {
    return BlocBuilder<CoreHrCubit, CoreHrState>(
      buildWhen: (prev, curr) =>
          prev.googleSignInInfo.status != curr.googleSignInInfo.status,
      builder: (context, state) {
