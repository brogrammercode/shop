...
export class InventoryService {
  async createSupplier(
    ...
    return inventoryRepo.createSupplier({
      branch_id: branchId,
      name,
      avatar,
      tax_number,
  ...
  async getSupplierById(id: string, branchId: string) {
    ...
    if (!supplier || supplier.branch_id !== branchId) {
      throw new NotFoundError(
        _INVENTORY_CONSTANTS._E_R_R_O_R_S.SUPPLIER_NOT_FOUND,
      );
    }
    return supplier;
  }
  ...
  async createItemCategory(
    ...
    return inventoryRepo.createItemCategory({
      branch_id: branchId,
      name,
      description,
      images,
  ...
  async createUOMConversion(
    ...
    return inventoryRepo.createUOMConversion({
      branch_id: branchId,
      from_uom_id,
      to_uom_id,
      factor,
  ...
  async createItemVariant(
    ...
    if (!finalSku) {
      ...
      const randomStr = Math.random()
        ...
        .substring(2, 6)
        .toUpperCase();
      finalSku = `${itemName}-${uomAbbr}-C${base_cost}-${randomStr}`;
    }
    ...
    return inventoryRepo.createItemVariant({
      branch_id: branchId,
      item_id,
      uom_id,
      sku: finalSku,
  ...
  async getVariantById(id: string, branchId: string) {
    ...
    if (!variant || variant.branch_id !== branchId) {
      throw new NotFoundError(
        _INVENTORY_CONSTANTS._E_R_R_O_R_S.VARIANT_NOT_FOUND,
      );
    }
    return variant;
  }
  ...
  async createPurchaseOrder(
    ...
    supplier_id: string,
    created_by: string,
    items: { variant_id: string; qty_ordered: number; unit_price: number }[],
    notes?: string,
  ) {
    let totalAmount = 0;
    ...
    const po = await inventoryRepo.createPurchaseOrder({
      branch_id: branchId,
      supplier_id,
      created_by,
    ...
    await inventoryRepo.createPOItems(
      poItems.map((item) => ({ ...item, po_id: po.id })),
    );
  ...
  async sendPurchaseOrder(id: string, branchId: string) {
    ...
    if (po.status !== "DRAFT") {
      throw new BadRequestError(
        _INVENTORY_CONSTANTS._E_R_R_O_R_S.INVALID_STATUS,
      );
    }
  ...
  async receivePurchaseOrder(
    ...
    for (const item of po.items) {
      const lastEntry = await inventoryRepo.getLastStockEntryForVariant(
        item.variant_id,
      );
  ...
  async createVendorReturn(
    ...
    const vendorReturn = await inventoryRepo.createVendorReturn({
      branch_id: branchId,
      po_id,
      return_reason,
      refund_value,
  ...
  async logUsage(
    ...
    const lastEntry =
      await inventoryRepo.getLastStockEntryForVariant(variant_id);
    const currentBalance = lastEntry?.running_balance || 0;

    if (currentBalance < quantity) {
      throw new BadRequestError(
        _INVENTORY_CONSTANTS._E_R_R_O_R_S.INSUFFICIENT_STOCK,
      );
    }
  ...
  async createStockTransfer(
    ...
    const transfer = await inventoryRepo.createStockTransfer({
      from_branch_id: fromBranchId,
      to_branch_id: toBranchId,
      dispatched_by,
      driver_name,
    });
    await inventoryRepo.createTransferItems(
      items.map((i) => ({ ...i, transfer_id: transfer.id })),
    );
  ...
  async getStockTransferById(id: string, branchId: string) {
    ...
    if (
      ...
      throw new NotFoundError(
        _INVENTORY_CONSTANTS._E_R_R_O_R_S.TRANSFER_NOT_FOUND,
      );
    }
    return transfer;
  }
